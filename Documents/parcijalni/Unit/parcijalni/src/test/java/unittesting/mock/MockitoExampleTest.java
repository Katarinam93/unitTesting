...
public class MockitoExampleTest {
	...
	@Test(expectedExceptions = RuntimeException.class) // we expect IOException during test execution
	public void testForIOException() throws IOException {
		...
		doThrow(new RuntimeException()).when(mockedExampleObject).doSomethingRisky();
	...
	public void testCallMethodThatIsNotMocked() {
		...
		when(mockedExampleObject.return12()).thenCallRealMethod();
		
		// non mocked method
		assertEquals(mockedExampleObject.return12(), 12);
	...
	public void testReturnValueDependentOnMethodParameter() {
		...
		when(c.compareTo(Mockito.anyString())).thenReturn(1);
		when(c.compareTo("some other text")).thenReturn(2);

		assertEquals(c.compareTo("some other other text"), 1);
	}
